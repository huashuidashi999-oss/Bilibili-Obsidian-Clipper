...
const elements = {
  ...
  recentPathsList: document.getElementById("recentPathsList"),
...
async function loadSettings() {
  ...
  elements.timestampMode.value = normalizeTimestampModeForSelect(settings.timestampMode, settings.includeTimestampInBody);
  ...
  renderRecentPaths(settings.recentCustomPaths);
}
...
function normalizeTimestampModeValue(value) {
...
function normalizeTimestampModeForSelect(mode, includeTimestampInBody) {
  const normalized = normalizeTimestampModeValue(mode);
  if (normalized === "media-extended" || normalized === "plain" || normalized === "hidden") {
    return normalized;
  }
  // backward compat from legacy boolean
...
function collectFormPayload() {
  ...
  return {
    ...
    timestampMode: normalizeTimestampModeValue(elements.timestampMode.value),
    ...
    recentCustomPaths: collectRecentPaths()
  };
}
...
function renderRecentPaths(paths) {
  const list = elements.recentPathsList;
  if (!list) return;
  const arr = Array.isArray(paths) ? paths.slice(0, 3) : ["", "", ""];
  ...
  arr.forEach((value, index) => {
    ...
    row.innerHTML = `
      <span class="path-index-badge">${index + 1}</span>
      <span class="path-drag-handle" draggable="true">☰</span>
      <input class="path-input" type="text" placeholder="例如：Clippings/Bilibili/课程名" value="${escapeAttribute(value)}" />
    `;

    const handle = row.querySelector(".path-drag-handle");
    ...
    handle.addEventListener("dragend", () => {
      ...
      list.querySelectorAll(".path-row").forEach((r) => r.classList.remove("drag-over"));
    });
    ...
    row.addEventListener("drop", (e) => {
      ...
      const rows = Array.from(list.querySelectorAll(".path-row"));
      const values = rows.map((r) => String(r.querySelector(".path-input")?.value || ""));
      const [moved] = values.splice(srcIndex, 1);
      values.splice(index, 0, moved);
      renderRecentPaths(values);
    });
...
function collectRecentPaths() {
  const list = elements.recentPathsList;
  if (!list) return ["", "", ""];
  return Array.from(list.querySelectorAll(".path-input")).map((input) => String(input.value || "").trim());
}
